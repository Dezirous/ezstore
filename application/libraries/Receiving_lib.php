...
class Receiving_lib
{
	...
	public function add_item($item_id, $quantity = 1, $item_location = NULL, $discount = 0, $discount_type = 0, $price = NULL, $description = NULL, $serialnumber = NULL, $receiving_quantity = NULL, $receiving_id = NULL, $include_deleted = FALSE)
	{
		...
		{
			$receiving_quantity_choices = array(1  => 'Unit');
		}
		else
		{
			$receiving_quantity_choices = array(
				to_quantity_decimals($item_info->receiving_quantity) => 'Pack',
				1  => 'Unit');
		}
		...
		$item = array($insertkey => array(
				...
				'line' => $insertkey,
				'name' => $item_info->name.' '.to_quantity_decimals($receiving_quantity).'s',
				...
				'quantity' => $quantity,
				'discount' => $item_info->discount_purchase, //$discount,
				'discount_type' => $discount_type,
				'in_stock' => $this->CI->Item_quantity->get_item_quantity($item_id, $item_location)->quantity,
				'price' => $price, //$item_info->pack_price, //
	...
	public function return_entire_receiving($receipt_receiving_id, $is_accept=0,$item_location=0)
	{
		//RECV #
		...
		{
			$this->add_item($row->item_id, ($is_accept==0?$row->quantity_purchased:-$row->quantity_purchased), $item_location>0?$item_location:$row->item_location, $row->discount, $row->discount_type, $row->item_unit_price, $row->description, $row->serialnumber, $row->receiving_quantity, $receiving_id, TRUE);
		}
