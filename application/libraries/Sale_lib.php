...
class Sale_lib
{
	...
	public function get_register_mode_options()
	{
		...
		$role = $this->CI->Employee->get_role();
		if($role != "POS"){
	...
	public function get_totals($taxes)
	{
		...
		foreach($this->get_cart() as $item)
		{
			...
			if($item['item_id'] == 1){
                $additional_charges_rate = $item['discount']/100;
            }
		}
		...
		if($additional_charges_rate > 0){
            $additional_charges = $total * $additional_charges_rate;			
        }
		...
		{
			$totals['cash_adjustment_amount'] = round($cash_total - $totals['total'] - $totals['additional_charges'], totals_decimals(), PHP_ROUND_HALF_UP);
		}
	...
	public function get_mode()
	{
		...
		// if($this->CI->session->userdata('user_role') == "POS")
		// {
		// 	$this->set_mode('sale_work_order');  
		// }else{			
		// 	if(!$this->CI->session->userdata('sales_mode'))                                                                                                                                                                                                                     
		// 	{                                                                                                                                                                                                                                                                   
		// 		$this->set_mode('sales_receipt');	
		// 	}  
		// }    
		// return $this->CI->session->userdata('sales_mode');
	}
	...
	public function add_item(&$item_id, $quantity = 1, $item_location, &$discount = 0.0, $discount_type = 0, $price_mode = PRICE_MODE_STANDARD, $kit_price_option = NULL, $kit_print_option = NULL, $price_override = NULL, $description = NULL, $serialnumber = NULL, $sale_id = NULL, $include_deleted = FALSE, $print_option = NULL, $line = NULL)
	{
		...
		$applied_discount = $item_info->discount_retail;
		...
		if(!$itemalreadyinsale || $item_info->is_serialized)
		{
			$item = array($insertkey => array(
					...
					'line' => $insertkey,
					'name' => $item_info->name.' '.$item_info->receiving_quantity.'s',
					...
					'quantity' => $quantity,
					'discount' => $item_info->discount_retail,
