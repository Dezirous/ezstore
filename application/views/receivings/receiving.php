	...
	<?php echo form_open($controller_name."/change_mode", array('id'=>'mode_form', 'class'=>'form-horizontal panel panel-default')); ?>
		<div class="panel-body form-group">
			<ul>
				...
				<li class="pull-left">
					<?php echo form_dropdown('mode', $modes, $mode, array('onchange'=>"$('#mode_form').submit();", 'class'=>'selectpicker show-menu-arrow', 'data-style'=>'btn-default btn-sm', 'data-width'=>'fit', 'tabindex'=>++$tabindex)); ?>
				</li>
				...
				?>
					...
					<li class="pull-left">
						<?php echo form_dropdown('stock_source', $stock_locations, $stock_source, array('onchange'=>"$('#mode_form').submit();", 'class'=>'selectpicker show-menu-arrow', 'data-style'=>'btn-default btn-sm', 'data-width'=>'fit', 'tabindex'=>++$tabindex)); ?>
					</li>
					...
					?>
						...
						<li class="pull-left">
							<?php echo form_dropdown('stock_destination', $stock_locations, $stock_destination, array('onchange'=>"$('#mode_form').submit();", 'class'=>'selectpicker show-menu-arrow', 'data-style'=>'btn-default btn-sm', 'data-width'=>'fit', 'tabindex'=>++$tabindex)); ?>
						</li>
				<?php
					}
				//}
	...
	<table class="sales_table_100" id="register">
		<thead>
			<tr>				
				...
				<th style="width:10%;">Unit Price<?php //echo $this->lang->line('receivings_cost'); ?></th>
				...
				foreach(array_reverse($cart, TRUE) as $line=>$item){
					echo form_open($controller_name."/edit_item/$line", array('class'=>'form-horizontal', 'id'=>'cart_'.$line)); ?>
						<tr>							
							...
							<?php if ($items_module_allowed && $mode !='requisition' && $role != "Supervisor"){ ?>
								<td><?php echo form_input(array('name'=>'price', 'class'=>'form-control input-sm', 'value'=>to_currency_no_money($item['price']),'onClick'=>'this.select();', 'tabindex'=>++$tabindex));?></td>
							<?php }else{ ?>
								<td>
									<?php echo $item['price']; ?>
							...
							<?php if($role != "Supervisor"){ ?>
								<td><?php echo form_input(array('name'=>'quantity', 'class'=>'form-control input-sm', 'type' => 'number', 'min'=>'1', 'value'=>to_quantity_decimals($item['quantity']),'onClick'=>'this.select();', 'tabindex'=>++$tabindex)); ?></td>
								<td><?php echo form_dropdown('receiving_quantity', $item['receiving_quantity_choices'], $item['receiving_quantity'], array('class'=>'form-control input-sm', 'tabindex'=>++$tabindex));?></td>
							<?php }else{ ?>
								<td>
									<?php echo to_quantity_decimals($item['quantity']); ?>
									<?php echo form_hidden('quantity', to_currency_no_money($item['quantity'])); ?>
								</td>
								<td>
									<?php echo $item['receiving_quantity'] > 1 ? 'Pack' : 'Unit'; ?>
									<?php echo form_hidden('receiving_quantity', $item['receiving_quantity']); ?>
								</td>
							<?php } ?>
							<?php if ($items_module_allowed && $mode!='requisition' && $role != "Supervisor"){ ?>
								<td>
									<div class="input-group">
										<?php echo form_input(array('name'=>'discount', 'class'=>'form-control input-sm', 'value'=>$item['discount_type'] ? to_currency_no_money($item['discount']) : to_decimals($item['discount']), 'onClick'=>'this.select();', 'tabindex'=>++$tabindex)); ?>
							...
							<?php } ?>
							<td><?php echo to_currency(($item['discount_type'] == PERCENT) ? $item['price']*$item['quantity']*$item['receiving_quantity'] - $item['price'] * $item['quantity'] * $item['receiving_quantity'] * $item['discount'] / 100 : $item['price']*$item['quantity']*$item['receiving_quantity'] - $item['discount']); ?></td> 
							...
							<div id="payment_details" >
								<table class="sales_table_100" >
									<tr>
										...
										<td><?php echo form_checkbox(array('name'=>'recv_print_after_sale', 'id'=>'recv_print_after_sale', 'class'=>'checkbox', 'value'=>1, 'checked'=>$print_after_sale)); ?></td>
									</tr>
									<?php // if ($mode == "receive") { ?>
										<tr>
											<td><?php echo $this->lang->line('receivings_reference');?></td>
											<td><?php echo form_input(array('name'=>'recv_reference', 'id'=>'recv_reference', 'class'=>'form-control input-sm', 'value'=>$reference, 'size'=>5, $reference = "" ? "" : 'readonly' => 'readonly')); ?></td>
										</tr>
									<?php //} ?>
									<tr>
										<td><?php echo $this->lang->line('sales_payment'); ?></td>
										<td><?php echo form_dropdown('payment_type', $payment_options, array(), array('id'=>'payment_types', 'class'=>'selectpicker show-menu-arrow', 'data-style'=>'btn-default btn-sm', 'data-width'=>'auto')); ?></td>
									</tr>
									<tr>
										<td><?php echo $this->lang->line('sales_amount_tendered'); ?></td>
										<td><?php echo form_input(array('name'=>'amount_tendered', 'value'=>'', 'class'=>'form-control input-sm', 'size'=>'5')); ?></td>
									</tr>
								</table>
							</div>
