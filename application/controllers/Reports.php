...
class Reports extends Secure_Controller
{
    ...
    public function date_input()
    {
        ...
        $stock_locations = $data = $this->xss_clean($this->Stock_location->get_allowed_locations('sales'));
        //$stock_locations['all'] = $this->lang->line('reports_all');
        $data['stock_locations'] = $stock_locations; //array_reverse($stock_locations, TRUE);
        $data['mode'] = 'sale';
        $data['sale_type_options'] = $this->get_sale_type_options();
    ...
    public function specific_customer_input()
    {
        ...
        foreach ($this->Customer->get_all()->result() as $customer) {
            if (isset($customer->company_name)) {
                $customers[$customer->person_id] = $this->xss_clean($customer->first_name . ' ' . $customer->last_name . ' (' . $customer->phone_number . ') ' . ' [ ' . $customer->company_name . ' ] ');
            } else {
                $customers[$customer->person_id] = $this->xss_clean($customer->first_name . ' ' . $customer->last_name . ' (' . $customer->phone_number . ')');
            }
        }
    ...
    public function brand_inventory_input()
    {
        $this->load->model('reports/Brand_inventory');
        $model = $this->Brand_inventory;
        ...
        $this->load->view('reports/brand_inventory_input', $data);
    }
    ...
    public function brand_inventory($location_id = '')
    {
        ...
        foreach ($report_data as $row) {
            $tabular_data[] = $this->xss_clean(array(
                ...
                'item_name' => $row['name'] . ' ' . $row['receiving_quantity'] . 's',
                'category' => $row['category'],
                'quantity' => to_quantity_decimals($row['quantity']),
                'reorder_level' => to_quantity_decimals($row['reorder_level']),
                'location_name' => $row['location_name'],
                'pack_price' => to_currency($row['pack_price']),
    ...
    public function inventory_summary($location_id = 'all', $item_count = 'all')
    {
        ...
        foreach ($report_data as $row) {
            $tabular_data[] = $this->xss_clean(array(
                'item_id' => $row['item_id'],                
                'item_name' => $row['name'],
                'category' => $row['category'] . ' ' . $row['receiving_quantity'] . 's',
                'quantity' => to_quantity_decimals($row['quantity']),
                'reorder_level' => to_quantity_decimals($row['reorder_level']),
                'location_name' => $row['location_name'],
