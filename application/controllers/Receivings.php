...
class Receivings extends Secure_Controller
{
	...
	public function change_mode()
	{
		...
		if($mode == "accept"){
			...
			$this->receiving_lib->set_stock_destination($stock_source);
		}else{
	...
	public function add()
	{
		...
		if($mode == "accept"){
			$item_location = $this->receiving_lib->get_stock_destination();
		}else{
		...
		if($mode == 'return' || $mode == 'accept'){
			if($this->Receiving->is_valid_receipt($item_id_or_number_or_item_kit_or_receipt)){
				$this->receiving_lib->return_entire_receiving($item_id_or_number_or_item_kit_or_receipt,($mode=="accept"?1:0),$item_location);
				$this->receiving_lib->set_reference("$item_id_or_number_or_item_kit_or_receipt");
			}else{
	...
	public function edit($receiving_id)
	{
		...
		$data['status'] = !empty($receiving_info['status']) ? 'Closed' : $receiving_info['status'];
	...
	public function complete()
	{
		...
		$data['receiving_id'] = 'RECV ' . $this->Receiving->save($data['cart'], $supplier_id, $employee_id, $data['comment'], $data['reference'], $data['payment_type'], $data['stock_location'],$data['mode']);
	...
	{
		$data['cart'] = $this->receiving_lib->get_cart();
		$role = $this->Employee->get_role();
		$data['role'] = $role;
		if(preg_match('/(Admin|Owner|Purchaser)/', $role)){
			$data['allowed'] = true;
		}else{
			$data['allowed'] = false;
		}
		...
		if($role != "Supervisor"){
			$data['modes'] = array('receive' => $this->lang->line('receivings_receiving'), 'return' => $this->lang->line('receivings_return'), 'accept' => "Accept");
			if($data['show_stock_locations']) 
			{
				$data['modes']['requisition'] = $this->lang->line('receivings_requisition');
			}
		}else{
			$data['modes'] = array('accept' => "Accept", 'return' => $this->lang->line('receivings_return'));
		}
		$data['mode'] = $this->receiving_lib->get_mode();
		$data['total'] = $this->receiving_lib->get_total();
		...
		$data['comment'] = $this->receiving_lib->get_comment();
		$data['reference'] = $this->receiving_lib->get_reference();
		$data['payment_options'] = $this->Receiving->get_payment_options($data['mode']);
		...
		// echo $data['stock_source'];
		// echo $data['stock_destination'];
		// echo $data['mode'];
	}
