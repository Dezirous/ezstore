...
class Items extends Secure_Controller
{
    ...
    public function view($item_id = NEW_ITEM)
    {
        ...
        if (!$this->Employee->has_grant('items_Inventory', $person_id)) {
            echo "Access denied.";
            exit;
        }
        ...
        $url = "/home/dezirete/domains/medsone/assets/products/";
        $picture = glob($url . $item_id . ".*");
        $picture = sizeof($picture) ? "https://cdn.deziretech.pk/" . basename($picture[0]) : "";
        ...
        $data['inventory'] = $allow_inventory; // $this->Employee->has_grant('items_Inventory', $this->Employee->get_logged_in_employee_info()->person_id);
    ...
    public function save($item_id = NEW_ITEM)
    {
        ...
        //Save item data
        $pack_price = parse_decimals($this->input->post('pack_price'));
        $unit_price = $pack_price / $receiving_quantity;
        $discount_retail = $this->input->post('discount_retail');
        $discount_purchase = $this->input->post('discount_purchase');
        $item_data = array(
            ...
            'pack_price' => parse_decimals($pack_price),
            'unit_price' => parse_decimals($unit_price),
            'cost_price' => parse_decimals($unit_price - ($unit_price) * ($discount_purchase / 100)),
            'discount_retail' => parse_decimals($discount_retail),
            'discount_purchase' => parse_decimals($discount_purchase),
        ...
        if (!empty($base64_string)) {
            $imageFile = '/home/dezirete/domains/medsone/assets/products/' . $item_id . '.jpg';
            $image = explode(',', $base64_string);
            $image = base64_decode($image[1]);
            $file_status = file_put_contents($imageFile, $image);
            if ($file_status) {
                $file_status = "Successfully Uploaded";
            } else {
