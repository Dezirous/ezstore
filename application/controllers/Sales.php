...
class Sales extends Secure_Controller
{
    ...
    public function item_search()
    {
        ...
        if ($this->sale_lib->get_mode() == 'sale' && $this->Sale->is_valid_receipt($receipt)) {
            // if a valid receipt or invoice was found the search term will be replaced with a receipt number (POS #)
            $suggestions[] = $receipt;
        }
    ...
    public function add()
    {
        ...
        $discount = $this->input->post('discount'); //$this->config->item('default_sales_discount');
        ...
        if ($mode == 'return' && $this->Sale->is_valid_receipt($item_id_or_number_or_item_kit_or_receipt)) {
            ...
            $pieces = explode(' ', $item_id_or_number_or_item_kit_or_receipt);
            if ($mode == 'sale' && $pieces[0] == 'WO' && $this->Sale->is_valid_receipt($item_id_or_number_or_item_kit_or_receipt)) {
                $this->sale_lib->return_entire_sale($item_id_or_number_or_item_kit_or_receipt);
                $this->session->set_userdata('sale_id', $pieces[1]);
            } elseif (!$this->sale_lib->add_item($item_id_or_number_or_item_kit_or_receipt, $quantity, $item_location, $discount, $discount_type, PRICE_MODE_STANDARD, NULL, NULL, $price)) {
                $data['error'] = $this->lang->line('sales_unable_to_add_item');
            } else {
    ...
    {     
        ...
        $data['additional_charges'] = $totals['additional_charges'];
        ...
        if ($this->sale_lib->is_invoice_mode()) {
            ...
            if ($sale_id == -1 && $this->Sale->check_work_order_number_exists($work_order_number)) {
                ...
                $data['barcode'] = NULL;
                $data['barcode'] = $this->barcode_lib->generate_receipt_barcode('WO ' . $data['sale_id_num']);
    ...
    private function _load_customer_data($customer_id, &$data, $stats = FALSE)
    {
        ...
        if ($customer_id != -1) {
            ...
            if (!empty($customer_info->company_name)) {
                ...
                $data['customer'] = $customer_info->first_name;
            }
    ...
    private function _load_sale_data($sale_id)
    {
        ...
        $data['stock_location_id'] = $this->sale_lib->get_sale_location();
        $store = $this->Stock_location->get_location($data['stock_location_id']);
        $data['store_address'] = $store->address; // $this->Stock_location->get_location($data['stock_location_id']);
        $data['store_contact'] = $store->contact; // $this->Stock_location->get_location_info($data['stock_location_id'],'contact');
    ...
    private function _reload($data = array())
    {
        ...
        $my_cart = $this->sale_lib->get_cart();
        $data['cart'] = $this->sale_lib->get_cart();
        ...
        if ($customer_info && $this->config->item('customer_reward_enable') == TRUE) {
            $data['payment_options'] = $this->Sale->get_payment_options(FALSE, TRUE);
        } else {
            ...
            if ($this->Sale->delete_list($sale_ids, $employee_id, $update_inventory)) {
                echo json_encode(array(
                    ...
                    'message' => $this->lang->line('sales_successfully_deleted') . ' ' .
                        count($sale_ids) . ' ' . $this->lang->line('sales_one_or_multiple'),
                    'ids' => $sale_ids
                ));
            } else {
    ...
    public function suspended()
    {
        $data = array();
        $customer_id = $this->sale_lib->get_customer();
        $location_id = $this->Employee->get_location();
        $data['suspended_sales'] = $this->xss_clean($this->Sale->get_all_suspended($customer_id, $location_id));
