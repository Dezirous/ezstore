...
function transform_headers($array, $readonly = FALSE, $editable = TRUE)
{
	...
	foreach($array as $element)
	{
		...
		$result[] = array('field' => key($element),
			...
			'escape' => !preg_match("/(edit|phone_number|email|messages|item_pic|role)/", key($element)) && !(isset($element['escape']) && !$element['escape']),
...
function get_person_data_row($person)
{
	...
	return array (
		...
		'role' => $person->role,
		'branch' => $person->location_name ? $person->location_name : 'All Branches',
...
function get_items_manage_table_headers()
{
	...
	$headers = array(
		...
		array('company_name' => "Supplier Name"),
		array('pack_price' => "Pack Price"),
		array('unit_price' => "Unit Price"),
		array('discount_retail' => "Retail %"),
		array('discount_purchase' => "Purchase %"),
...
function get_item_data_row($item)
{
	...
	$columns = array (
		...
		'name' => $item->name." ".$item->receiving_quantity."s",
		'category' => $item->category,
		'company_name' => $item->company_name,
		'pack_price' => to_currency($item->pack_price),
		'unit_price' => to_currency($item->unit_price),
		'discount_retail' => parse_decimals($item->discount_retail),
		'discount_purchase' => parse_decimals($item->discount_purchase),
	...
	$icons = array(
		'inventory' => anchor($controller_name."/inventory/$item->item_id", '<span class="glyphicon glyphicon-retweet"></span>',
