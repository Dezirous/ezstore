	...
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'items.name', $order = 'asc', $count_only = FALSE)
	{
		...
		else
		{
			...
			$this->db->select('MAX(items.pack_price) AS pack_price');
			$this->db->select('MAX(items.unit_price) AS unit_price');
			$this->db->select('MAX(items.discount_retail) AS discount_retail');
			$this->db->select('MAX(items.discount_purchase) AS discount_purchase');
