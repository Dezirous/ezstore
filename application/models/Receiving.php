...
class Receiving extends CI_Model
{
    ...
    public function save($items, $supplier_id, $employee_id, $comment, $reference, $payment_type, $receiving_id = FALSE, $mode="")
    {
        if (count($items) == 0) {
            return -1;
        }
        ...
        $receivings_data = array(
            ...
            'payment_type' => ($payment_type == '' ? 'Transfer' : $payment_type),
            'comment' => $comment,
            'reference' => $reference,
            'status' => $mode == 'requisition' ? 'Pending' : 'Closed'
        );
        ...
        //$is_valid = 0;
        if (!empty($reference) && $mode == "accept") {
            $pieces = explode(' ', $reference);
            if (count($pieces) == 2 && preg_match('/(RECV|KIT)/', $pieces[0])) {
                //$is_valid = $this->exists($pieces[1]);
                $this->db->set('status', "Closed");
                $this->db->set('reference', $recv_remarks);
    ...
    public function get_payment_options($mode = "")
    {
        if($mode == "accept"){
            return array(
                "Transfer" => "Transfer"
        ...
        $this->db->query(
			...
			(
				SELECT 
					...
					MAX(receivings.status) AS status,
