...
class Employee extends Person
{
	...
	*/
	...
	public function get_location()
	{
		$employee_id = $this->session->userdata('person_id');
        $employee_info = $this->Employee->get_info($employee_id);
		return $employee_info->user_location_id;
	}
	...
	*/
	public function get_info($employee_id)
	{
		...
		$this->db->join('people', 'people.person_id = employees.person_id');
		$this->db->where('employees.person_id', $employee_id);
		$this->db->join('stock_locations sl', 'sl.location_id = employees.user_location_id','left');
		$query = $this->db->get();
	...
	*/
	public function get_search_suggestions($search, $include_deleted = FALSE, $limit = 5)
	{
		...
		$this->db->group_start();
			...
			$this->db->or_like('CONCAT(first_name, " ", first_name)', $search);
		$this->db->group_end();
		...
		{
			$suggestions[] = array('value' => $row->person_id, 'label' => $row->first_name.' '.$row->first_name);
		}
	...
	*/
	...
	{
		return $this->search($search, 0, 0, 'first_name', 'asc', TRUE);
	}

	/*
	Performs a search on employees
	*/
	public function search($search, $rows = 0, $limit_from = 0, $sort = 'first_name', $order = 'asc', $count_only = FALSE)
	{
		// get_found_rows case
		if($count_only == TRUE)
		{
		...
		$this->db->join('people', 'employees.person_id = people.person_id');
		$this->db->join('stock_locations', 'employees.user_location_id = stock_locations.location_id','left');
		$this->db->group_start();
			$this->db->like('first_name', $search);
			$this->db->or_like('first_name', $search);
	...
	public function login($username, $password)
	{
		...
		if($query->num_rows() === 1)
		{
			...
			if($row->hash_version === '1' && $row->password === md5($password))
			{
				...
				//$this->session->set_userdata('person_location', $row->user_location_id);
				//$this->session->set_userdata('person_role', $row->role);
