...
class Sale extends CI_Model
{
    ...
    public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'sales.sale_time', $order = 'desc', $count_only = FALSE)
    {
        ...
        if ($count_only == TRUE) {
            ...
            $this->db->select('
					...
					MAX(CONCAT(customer_p.first_name, " ", customer_p.last_name)) AS customer_name,
					MAX(concat(employees.first_name, " ", employees.last_name)) AS employee,
        ...
        $this->db->join('sales AS sales', 'sales_items.sale_id = sales.sale_id', 'inner');
        $this->db->join('people AS customer_p', 'sales.customer_id = customer_p.person_id', 'LEFT');
        $this->db->join('people AS employees', 'sales.employee_id = employees.person_id', 'LEFT');
        $this->db->join('customers AS customer', 'sales.customer_id = customer.person_id', 'LEFT');
        $this->db->join('sales_payments_temp AS payments', 'sales.sale_id = payments.sale_id', 'LEFT OUTER');
        $this->db->join(
            'sales_items_taxes_temp AS sales_items_taxes',
        ...
        if (!empty($search)) {
            if ($filters['is_valid_receipt'] != FALSE) {
                ...
                // employee name
                $this->db->or_like('employees.first_name', $search);
                $this->db->group_end();
            }
        }
    ...
    public function get_payments_summary($search, $filters)
    {
        ...
        if ($filters['location_id'] > 0) {
            $this->db->where('sales.sale_location_id', $filters['location_id']);
        }
    ...
    public function is_valid_receipt(&$receipt_sale_id)
    {
        if (!empty($receipt_sale_id)) {
            ...
            if (count($pieces) == 2) {
                if (preg_match('/(POS)/i', $pieces[0]) || preg_match('/(WO)/i', $pieces[0])) {
    ...
    public function save(
        ...
        #get sale_location_id from selected items location 
        $sale_location_id = 0;
        foreach ($items as $line => $item) {
            $sale_location_id = $item['item_location'];
        }
        ...
        $sales_data = array(
            ...
            'sale_status' => $sale_status,
            'sale_type' => $sale_type,
            'sale_location_id' => $sale_location_id,
            'sale_location_ip' => $IP
        );
        ...
        foreach ($payments as $payment_id => $payment) {
            ...
            if ($payment['payment_amount'] > 0) {
                $sales_payments_data = array(
                    'sale_id' => $sale_id,
                    'payment_type' => $payment['payment_type'],
                    'additional_charges' => $payment['payment_amount'] * $additional_charges,
                    'payment_amount' => $payment['payment_amount'],
                    'cash_refund' => $payment['cash_refund'],
                    'cash_adjustment' => $payment['cash_adjustment'],
                    'employee_id' => $employee_id
     ...
     */
    public function get_payment_options($giftcard = FALSE, $reward_points = FALSE)
    {
        //$payments = get_payment_options();        

        if ($this->sale_lib->get_mode() == 'sale_work_order') {
            $payments[$this->lang->line('sales_due')] = "Due";
            ...
            $payments[$this->lang->line('sales_gateway')] = $this->lang->line('sales_gateway');
     ...
     */
    public function get_all_suspended($customer_id = NULL, $location_id = 0)
    {   
        $location_filter = $location_id > 0 ? ' AND sale_location_id=' . $location_id : '';
        $customer_filter = $customer_id > 0 ? ' AND customer_id = ' . $customer_id : '';
        ...
        //         . $this->db->dbprefix('sales') . ' where sale_status = ' . SUSPENDED . '  order by sale_id desc limit 50');
        // } else {
        ...
        //         . $this->db->dbprefix('sales') . ' where sale_status = ' . SUSPENDED . ' AND customer_id = ' . $customer_id . ' order by sale_id desc limit 50');
        // }

        $query = $this->db->query("SELECT sale_id, case when sale_type = '" . SALE_TYPE_QUOTE . "' THEN quote_number WHEN sale_type = '" . SALE_TYPE_WORK_ORDER . "' THEN work_order_number else sale_id end as doc_id, sale_id as suspended_sale_id, sale_status, sale_time, dinner_table_id, customer_id, employee_id, comment FROM "
        . $this->db->dbprefix('sales') . ' where sale_status = ' . SUSPENDED . $customer_filter . $location_filter . '  order by sale_id desc limit 25');
